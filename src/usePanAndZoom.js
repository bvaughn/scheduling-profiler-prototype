    ...
    case 'mouse-move':
      ...
      if (state.isDragging) {
        return {
          ...
          offsetX: clamp(
            0,
            getMaxOffsetX(payload.canvas.width, state.unscaledContentWidth, state.zoomLevel),
    ...
    case 'wheel':
      ...
      if (absDeltaX > absDeltaY) {
        ...
        if (absDeltaY > WHEEL_DELTA_THRESHOLD) {
          ...
          // Given: x position = ( timestamp * zoom level ) - x offset
          // Then: x offset = ( timestamp * zoom level ) - x position
...
export default function usePanAndZoom(canvasRef, unscaledContentWidth) {
  ...
  useEffect(() => {
    ...
    const initialZoomLevel = Math.max(MIN_ZOOM_LEVEL, width / unscaledContentWidth);
